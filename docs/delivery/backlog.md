...
| 7 | Developer | As a developer, I want service-type-to-container-template mappings so that each diagram node type translates to the correct Docker container configuration | Agreed | Container templates exist for all 5 service types; templates produce correct images/configs; service dependency ordering enforced at startup; port allocation avoids conflicts |
