...
| 12 | User | As a user, I want deploy operations to handle common issues gracefully and show me clear error messages so that I can understand and fix problems without reading Docker internals | Proposed | Update deploy allocates ports without conflicting with running containers; errors are user-friendly (no raw Docker daemon messages); integration tests with real Docker verify common flows; tests gated behind build tag |
...
| 12 | Deploy Robustness & Hardening | [View Details](./12/prd.md) |
...
| 2026-02-20 08:20:41 | 9 | Status Change | Agreed → InProgress. Started implementation. | AI_Agent |
| 20260220-094604 | 12 | Created | PBI created from feature request: deploy robustness — fix port conflicts on update deploy, user-friendly error messages, real Docker integration tests. | AI_Agent |
| 2026-02-20 16:08:41 | 9 | Status Change | InProgress → Done. All 7 tasks completed and verified. | AI_Agent |
