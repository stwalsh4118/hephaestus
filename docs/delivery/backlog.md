...
| 8 | Developer | As a developer, I want a mock API system that generates OpenAPI specs from user-defined endpoints and serves them via Prism so that API services return realistic mock data | Agreed | Endpoint definitions translate to valid OpenAPI 3.0 specs; Prism container reads mounted spec and serves endpoints; mock responses contain realistic fake data; cross-service HTTP calls resolve within Docker network |
